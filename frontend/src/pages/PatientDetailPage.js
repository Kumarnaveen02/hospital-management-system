      ...
      <Card className="border border-border shadow-none">
        <CardContent className="p-6">
          <div className="flex items-start gap-6">
            ...
            <div className="w-16 h-16 bg-[#0EA5E9] rounded-xl flex items-center justify-center text-white text-2xl font-bold shrink-0">
              {patient.name?.charAt(0)?.toUpperCase()}
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <User size={14} />
                  {patient.gender}, {patient.age} {t('years')}
                </div>
      ...
      <Tabs defaultValue="appointments" className="space-y-4">
        ...
        <TabsList className="bg-card border border-border">
          ...
          <TabsTrigger value="appointments">
            ...
            {t('appointments')} ({patient.appointments?.length || 0})
          </TabsTrigger>

          <TabsTrigger value="prescriptions">
            <FileText size={14} className="mr-1.5" />
            {t('prescriptions')} ({patient.prescriptions?.length || 0})
          </TabsTrigger>

          <TabsTrigger value="bills">
            <Receipt size={14} className="mr-1.5" />
            {t('billing')} ({patient.bills?.length || 0})
          </TabsTrigger>
            ...
            <CardContent className="p-6">
              {patient.appointments?.length > 0 ? (
                patient.appointments.map((a) => (
                  <div key={a.id} className="py-3 border-b">
                    <p>{a.date} at {a.time_slot}</p>
            ...
            <CardContent className="p-6">
              {patient.prescriptions?.length > 0 ? (
                patient.prescriptions.map((rx) => (
                  <div key={rx.id} className="py-3 border-b">
                    <p>{rx.id}</p>
                    <p>{rx.diagnosis}</p>
                  </div>
                ))
              ) : (
                <p>{t('noPrescriptionsYet')}</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
